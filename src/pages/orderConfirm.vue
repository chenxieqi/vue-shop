<template>
  <div class="order-confirm">
    <order-header :title="オーダー確認"></order-header>
      ...
      <div class="container">
        <div class="order-box">
          <div class="item-address">
            <h2 class="addr-title">アドレス帳</h2>
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{'checked':checkedIndex == index}" v-for="(item,index) in list" :key="index" @click="checkedIndex = index">
              ...
              <div class="addr-add" @click="addAddressModal">
          ...
          <div class="detail">
            <div class="item">
              <span class="item-name">商品数：</span>
              <span class="item-val">{{count}}点</span>
            </div>
            <div class="item">
              <span class="item-name">商品合計：</span>
              <span class="item-val">＄{{cartTotalPrice}}</span>
            </div>
            <div class="item-total">
              <span class="item-name">合計金額：</span>
              <span class="item-val">＄{{cartTotalPrice}}</span>
            </div>
          </div>
          <div class="btn-group">
            <a href="/#/cart" class="btn btn-default btn-large">買い物かごに戻る</a>
            <a href="javascript:;" class="btn btn-large" @click="oderSubmit">支払いへ</a>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <template v-slot:body>
        <div class="edit-wrap">
          <div class="item">
            <input type="text" class="input" placeholder="名前" v-model="checkedAdress.receiverName">
            <input type="text" class="input" placeholder="電話番号" v-model="checkedAdress.receiverMobile">
          </div>
          <div class="item">
            <select name="province" v-model="checkedAdress.receiverProvince">
              <option value="関東">関東</option>
              <option value="関西">関西</option>
            </select>
            <select name="city" v-model="checkedAdress.receiverCity">
              <option value="東京">東京</option>
              <option value="大阪">大阪</option>
            </select>
            <select name="district" v-model="checkedAdress.receiverDistrict">
              <option value="東京">港区</option>
              <option value="東京">新宿区</option>
              <option value="東京">渋谷区</option>
              <option value="大阪">中央区</option>
              <option value="大阪">西区</option>
              <option value="大阪">浪速区</option>
            </select>
          </div>
          <div class="item">
            <textarea name="street" v-model="checkedAdress.receiverAddress"></textarea>
          </div>
          <div class="item">
            <input type="text" class="input" placeholder="郵便番号" v-model="checkedAdress.receiverZip">
          </div>
        </div>
      </template>
    </modal>
...
export default{
  ...
  data(){
    return {
      list:[],//アドレスリスト
      cartList:[],//商品リスト
      cartTotalPrice:0,//合計金額
      count:0,//商品数
      checkedAdress:{},//選択されたアドレス
      userAction:'',//アクション 0：追加 1：編集 2：削除
      showDelModal:false,//削除モーダルウインドウ表示
      showEditModal:false,//編集モーダルウィンドウ表示
      checkedIndex:0//選択されたアドレスのインデックス
    }
  },
  components:{
    Modal,
  ...
  methods:{
    ...
    //アドレス編集ウインドウ表示
    editAddressModal(addr){
      this.showEditModal = true
      this.checkedAdress = addr
      this.userAction = 1
    },
    //アドレス削除
    delAddress(addr){
    ...
    //アドレス操作をコミット
    submitAddress(){
      let {checkedAdress,userAction} = this;
      ...
      if (userAction === 0) {
        ...
        method = 'put'
        url = `/shippings/${checkedAdress.id}`
      }else if (userAction === 2) {
        method = 'delete'
        url = `/shippings/${checkedAdress.id}`
      }
      if (userAction === 0 || userAction === 1) {
        let { receiverName, receiverMobile, receiverProvince, receiverCity, receiverDistrict, receiverAddress, receiverZip} = checkedAdress;
        let errMsg='';
        if (!receiverName) {
          errMsg = '名前を入力してください'
    ...
    //オーダーサブミット
    oderSubmit(){
      let addr = this.list[this.checkedIndex]
      if (!addr) {
        this.$message.error('配送先を選択してください')
        return
      }
